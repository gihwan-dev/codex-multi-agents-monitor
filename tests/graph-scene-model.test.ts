...
describe("buildGraphSceneModel", () => {
  ...
  it("builds an event reveal target for event selections", () => {
    ...
    expect(scene.selectionRevealTarget).toEqual({
      kind: "event",
      eventId: "fix2-blocked",
    });
  });

  it("builds an edge reveal target spanning both endpoints", () => {
    const dataset = FIXTURE_DATASETS.find((item) => item.run.traceId === "trace-fix-002");
    expect(dataset).toBeDefined();
    if (!dataset) {
      throw new Error("waiting-chain fixture missing");
  ...
  it("builds an artifact reveal target from the producer event", () => {
    const dataset = FIXTURE_DATASETS.find((item) => item.run.traceId === "trace-fix-001");
    expect(dataset).toBeDefined();
    if (!dataset) {
