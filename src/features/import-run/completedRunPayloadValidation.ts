...
type ValidatedCompletedRunPayload = Pick<
  RawImportPayload,
...
function assertOptionalString(record: Record<string, unknown>, key: string) {
  const value = record[key];
...
function validatePayloadReferences(payload: ValidatedCompletedRunPayload) {
