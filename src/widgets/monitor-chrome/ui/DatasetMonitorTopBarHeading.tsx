...
export function DatasetMonitorTopBarHeading({
  ...
  return (
    <div className="grid min-w-0 gap-1.5">
      ...
      <p className="truncate text-[0.82rem] text-muted-foreground">{dataset.project.name} / {dataset.session.title}</p>
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        <h1 className="min-w-0 truncate text-[clamp(1.18rem,2vw,1.5rem)] font-semibold">{dataset.run.title}</h1>
