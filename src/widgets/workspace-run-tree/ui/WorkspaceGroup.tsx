...
export function WorkspaceGroup({
  ...
  return (
    <section data-slot="workspace-group" data-workspace-id={workspace.id} data-expanded={expanded ? "true" : "false"} className="grid gap-1 border-b border-white/6 pb-2">
      ...
      <div data-slot="workspace-group-body" data-state={state} aria-hidden={!expanded} className="grid grid-rows-[0fr] opacity-0 transition-[grid-template-rows,opacity] duration-[var(--duration-normal)] ease-[var(--easing-emphasized)] data-[state=open]:grid-rows-[1fr] data-[state=open]:opacity-100">
