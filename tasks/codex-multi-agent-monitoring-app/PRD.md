...
- `REQ-003` Live Monitor 상세 화면은 `User | Main | Sub-agent...` lane과 시간 축을 갖는 sequence timeline을 제공해야 한다.
...
|  - live session A       |                                                         |
|  - archived session B   | time ------------------------------------------------>  |
| resume-with-ai          | User | Main | Agent Writer | Agent Reviewer            |
|  - live session C       |  Q1  |---- plan ----|                                   |
|                         |      | tool:rg      |                                   |
|                         |      | spawn ------>|==== work span ====|               |
|                         |      | wait --------|------------------>|               |
|                         |      | result <-----|                                   |
