...
- [x] **UX-36**: SessionScoreEditorDialog hardcoded dark-only `bg-[rgb(14_18_28)]` → `--gradient-dialog-surface` 토큰 사용 (이전 세션)
...
- [x] **UX-48**: MonitorShortcutsDialog 미구현 `I` 단축키 줄 제거 (이번 세션)
- [x] **FE-61**: useScanInvocations scanError 상태 추가 + UI 에러 배너 (이번 세션)
- [x] **A11Y-4**: GraphEventCardContent 토큰 뱃지 aria-label + role="img" 추가 (이번 세션)
- [x] **UX-43**: FollowLiveAction aria-pressed 추가 (이전 세션)
- [x] **UX-44**: CausalGraphLaneStrip 레인 header → section + aria-label (이전 세션)
- [x] **UX-45**: GraphGapRow time cell gap label 렌더링 (이전 세션)
- [x] **UX-46**: MonitorRail 부분 에러 표시 + role="alert" (이전 세션)
- [x] **UX-47**: MonitorTopBarBadges liveConnection 사용자 친화 레이블 + aria-label (이전 세션)
...
- [x] **COR-9-be**: `path_is_allowed` str::starts_with → Path::starts_with 세그먼트 비교 (이전 세션)
- [x] **ERR-3-be**: `record_event` best-effort 패턴 — audit 실패가 트랜잭션에 영향 주지 않음 (이전 세션)
- [x] **FE-55**: `finishSessionScoreLoad` active 가드 제거 — 취소 후 stuck loading 해소 (이전 세션)
- [x] **ARCH-3**: `MonitorTopBarShell` monitor-chrome/index.ts export 추가 + internal bypass 제거 (이전 세션)
- [x] **ARCH-4**: `StatusGlyphMark` shared/ui/monitor/index.ts export 추가 + internal bypass 제거 (이전 세션)
- [x] **ARCH-5**: `SkillStatusBadge.constants` shared/ui/monitor/index.ts export 추가 + internal bypass 제거 (이전 세션)
- [x] **UX-42**: `EvalRunPicker` disabled prop + `EvalCompareControls` case 미선택 시 picker 비활성화 (이전 세션)
- [x] **SEC-3-be**: `is_main_claude_session_path` symlink 가드 추가 (이전 세션)
- [x] **COR-7-be**: `load_all_session_score_records` best-effort 패턴 — corrupt 파일 로깅 후 건너뛰기 (이전 세션)
- [x] **COR-8-be**: `ClaudeUsageMetrics` 누적 토큰 추적 — last/total 구분 (이전 세션)
- [x] **CON-2-be**: live session cancel flag Acquire/Release ordering (이전 세션)
- [x] **FE-54**: CopyButton setTimeout cleanup on unmount (이전 세션)
- [x] **LINT-1**: `summarySelectors.ts` 파일 길이 초과 + `collectAnomalyCandidates` 인지 복잡도 → anomalyJumps.ts 분리 (이전 세션)
- [x] **UX-36**: SessionScoreEditorDialog dark-only bg → gradient-dialog-surface 토큰 (이전 세션)
- [x] **SEC-2**: symlink .jsonl 파일 스캔 우회 차단 — `!path.is_symlink()` 가드 추가 (이전 세션)
- [x] **COR-1**: `build_profile_revision_summary` expect 패닉 → Option ��환 (이전 세션)
- [x] **COR-2**: `load_all_experiment_details` 단일 파일 오류 시 전체 중단 → 로깅 후 건너뛰기 (이전 세션)
- [x] **FE-38**: `collectAnomalyCandidates` O(n log n)×2 → O(n) 단일 순회 (이전 세션)
- [x] **FE-39**: `findLastHandoff` 렌더당 2회 호출 → 1회로 통합 (이전 세션)
...
- [x] **N5a**: EvalExperimentListRow aria-label 누락 (이전 세션)
- [x] **N5b**: EvalCaseListPanel CaseRow aria-label 누락 (이전 세션)
- [x] **N5c**: CausalInspectorJumpButton aria-label 누락 (이전 세션)
- [x] **N5d**: SkillActivityTopBar back button aria-label 누락 (이전 세션)
- [x] **N5e**: SkillActivityToolbar Select aria-labels 누락 (이전 세션)
...
- [x] **UX-30**: StatusChipGlyph + GraphStatusDot AC-004 shape 규칙 적용 — running=filled, waiting=hollow, blocked=slashed, failed=diamond, done=small (이전 세션)
- [x] **UX-31**: EvalRunPicker placeholder option disabled 추가 (이전 세션)
- [x] **UX-32**: EvalRunGradeSection badge truncate + title 추가 (이전 세션)
- [x] **UX-33**: SkillActivityToolbar scanLoading 시 모든 Input/Select disabled (이전 세션)
- [x] **UX-34**: SkillActivityLegend summary focus-visible ring 추가 (이전 세션)
- [x] **UX-35**: LoadingProgress role="progressbar" + aria-label 추가 (이전 세션)
- [x] **FE-22**: findLastHandoff O(E×H) → Map pre-computation O(E+H) (이전 세션)
- [x] **FE-24**: Worker onmessageerror 핸들러 추가 — 메모리 누수 방지 (이전 세션)
- [x] **COR-1-be**: read_tail_buffer UTF-8 경계 안전 처리 — from_utf8_lossy (이전 세션)
- [x] **ARCH-1-be**: validate_head_reference Component::Normal 강화 + null byte 거부 (이전 세션)
- [x] **COR-5-be**: load_recent_session_index 에러 로깅 — unwrap_or_default → match + eprintln (이전 세션)
