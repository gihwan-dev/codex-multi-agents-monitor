...
이 문서는 현재 저장소의 프론트엔드 구조를 `app / pages / widgets / features / entities / shared` 6레이어 기준으로 다시 정리한 아키텍처 기준 문서다. 목표는 화면 조합, 사용자 액션, 도메인 모델, 공용 유틸의 책임을 분리해 `src/app`와 `src/shared/domain`에 집중된 결합도를 낮추는 것이다. `processes` 레이어는 v0.1 범위에 넣지 않는다. 모든 slice는 자기 public API만 노출하고, 상위 레이어는 하위 레이어의 `index.ts`를 통해서만 import한다.
...
- After: `app`는 bootstrap만, `pages/monitor`는 page composition과 page-local orchestration, `widgets`는 화면 블록, `features`는 사용자 액션, `entities`는 핵심 모델, `shared`는 공용 primitive와 util만 담당한다.
- After: `shared/domain`은 안정 레이어가 아니라 이행 구간으로 본다. 필요한 항목은 `entities/*`와 widget-local model로 이동한다.
...
- 호환성 영향은 낮추는 쪽으로 설계하지만, `shared/domain`과 `src/app/session-log-loader`는 최종적으로 사라져야 한다.
- 운영 리스크는 import churn과 selector 이동 중복이다. 이 때문에 한 slice마다 boundary family 하나만 옮기고 caller를 같이 갱신한다.

## 5) 후속 작업
- `src/app`의 page orchestration을 `pages/monitor`로 이동한다.
- `src/features/run-list`, `src/features/run-detail`, `src/features/inspector`를 `widgets`로 승격한다.
- `src/shared/domain`의 selector와 DTO를 `entities`와 widget-local model로 분해한다.
- `src/app/app.css`를 widget CSS로 분리하고 legacy shell CSS만 남긴다.
